...
- `mcp__whw__find_similar_findings` — vector search prior `Finding` nodes by an embedding you
  pass (typically the embedding of your working hypothesis text).
...
- `Bash` (restricted to `docker run ... <tools_image> ...`) — invoke language static tools
  installed in the tools Docker image. Mount source read-only.
...
4. **Look for novel vulnerabilities.** Beyond the checklist, identify invariant violations
...
5. **For each finding** call `add_finding` with:
...
6. **Stop** when your hypotheses are exhausted. Do **not** speculate beyond evidence.
