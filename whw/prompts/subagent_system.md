...
callees, an upstream **TRUST PATH** that shows whether the function is reachable from any
untrusted entry, and any sparse USAGE/WRITES edges that touch it) and passed them to you
in the user task message.
...
- `mcp__whw__find_upstream_entrypoints` — walk CALLS backward to any flagged untrusted entry
  (entrypoint_kind set, trust_level=UNTRUSTED, or handler/route/webhook/fuzzer-shaped name).
  Use when the pre-fetched TRUST PATH block is empty or you want deeper hops.
...
2. **TRUST-PATH WALK — does this function inherit an unguarded untrusted reach?** Look at
   the `TRUST PATH` block in your slice. Each line lists an untrusted entry (handler /
   route / webhook / fuzzer / `entrypoint_kind`-marked / `trust_level=UNTRUSTED`) and the
   shortest CALLS chain to your target. For every chain, trace it node-by-node and ask:
   between the entry and the target, is there an explicit authentication, authorization,
   input-validation, or sanitisation step? If the chain is short (≤ 2 hops) and unguarded,
   the target inherits the entry's untrusted reach — call this out in your rationale and
   prefer findings of class `AUTH_BYPASS`, `CSRF`, `SSRF`, `PATH_TRAVERSAL`, `CMD_INJECTION`,
   or `MISSING_BOUNDS_CHECK` over generic NOVEL. If the TRUST PATH block is empty but you
   suspect a reachable untrusted source the harness missed, call
   `find_upstream_entrypoints` with a deeper `max_depth`. An empty TRUST PATH at depth ≥ 3
   is a meaningful **negative** signal — it raises the bar for filing auth-bypass-shaped
   findings on this function.
3. **Check priors.** For each working hypothesis: draft a one-line `summary` of the
...
4. **Look for classic vulnerabilities.** Specifically:
   ...
   - **Trust-path-aware**: auth/authz bypass, CSRF, SSRF, open-redirect — when the
     TRUST PATH block from step 2 shows an unguarded chain.
5. **Run static tools (if available).** If the task message has a "STATIC ANALYSIS
...
6. **Look for novel vulnerabilities.** Beyond the checklist, identify invariant violations
...
7. **For each finding** call `add_finding` with:
   - `vuln_class` ∈ {`OOB_READ`, `OOB_WRITE`, `USE_AFTER_FREE`, `DOUBLE_FREE`,
     ...
     `UNCHECKED_ALLOC`, `ERROR_PATH_LEAK`, `AUTH_BYPASS`, `CSRF`, `SSRF`, `NOVEL`}
...
8. **Stop** when your hypotheses are exhausted. Do **not** speculate beyond evidence.
